...
export function NeonLabTheme({ profile, links, socials, videos, banners, track }: BioThemeProps) {
  ...
  const accent = profile.button_color || '#FF2D95';
  const bg = profile.bg_color || '#050012';
  const text = profile.text_color || '#F5E9FF';
  const secondary = s.secondary || '#22E9FF';
    ...
    <div className="min-h-screen relative overflow-hidden" style={{ backgroundColor: bg, color: text }}>
      ...
      <div className="relative max-w-md mx-auto px-5 pt-[72px] pb-24">
        <div className="flex flex-col items-center text-center">
          ...
          <p className="text-sm mt-1" style={{ color: secondary, textShadow: `0 0 ${s.glow / 5}px ${secondary}` }}>@{profile.username}</p>
        ...
        <div className="mt-8 flex flex-col gap-3">
          ...
          {banners?.map((b: any) => {
            const inner = (
              <div className="overflow-hidden" style={{ border: `1px solid ${accent}`, boxShadow: `0 0 ${s.glow / 2}px ${accent}55`, height: b.size === 'sm' ? 100 : b.size === 'lg' ? 240 : 160 }}>
