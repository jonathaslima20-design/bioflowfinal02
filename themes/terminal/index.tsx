...
export function TerminalTheme({ profile, links, socials, videos, banners, track }: BioThemeProps) {
  ...
  const caret = s.caretColor || '#34D399';
    ...
    <div className="min-h-screen pt-[72px] pb-24 px-4" style={{ backgroundColor: bg, color: text, fontFamily: '"JetBrains Mono", "Fira Code", ui-monospace, monospace' }}>
      <div className="max-w-md mx-auto rounded-lg overflow-hidden" style={{ border: '1px solid #ffffff15', boxShadow: '0 20px 50px rgba(0,0,0,0.4)' }}>
        <div className="flex items-center gap-2 px-3 py-2 border-b" style={{ background: '#0006', borderColor: '#ffffff10' }}>
          ...
          <div className="flex-1 text-center text-xs opacity-70">bioflowzy@{profile.username} — ~/links</div>
        </div>
        <div className="p-5 text-sm leading-relaxed">
          {s.ascii && (
            <pre className="text-[10px] leading-tight mb-4 opacity-80" style={{ color: accent }}>
{`  ____  _       _____ _             _
 ...
 |____/|_|\\___/|_|   |_|\\___/ \\___/(_)`}
          ...
          <div className="mb-3">
            ...
            <span className="opacity-80">whoami</span>
          </div>
          <div className="flex items-start gap-3 mb-3">
            ...
            <div>
              <div style={{ color: caret, fontWeight: 700 }}>{profile.display_name || profile.username}</div>
              <div className="opacity-70">@{profile.username}</div>
          ...
          {socials?.length > 0 && (
            <div className="mb-4">
              <div><span style={{ color: accent }}>{s.prompt} </span><span className="opacity-80">cat socials.json</span></div>
          ...
          <div className="mb-2"><span style={{ color: accent }}>{s.prompt} </span><span className="opacity-80">ls -la ./links</span></div>
          ...
          {banners?.map((b: any) => {
            const inner = (
              <div className="mt-3 rounded overflow-hidden" style={{ border: `1px solid ${accent}44`, height: b.size === 'sm' ? 90 : b.size === 'lg' ? 220 : 150 }}>
